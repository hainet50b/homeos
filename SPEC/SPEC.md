...
- **Windows shell selection**: prefer `pwsh` (PowerShell 7+); fall back to `powershell.exe` (Windows PowerShell 5.1) when `pwsh` is absent. The fallback is announced in the plan display (`(running under Windows PowerShell 5.1; PowerShell 7 recommended)`) so the user sees the constraint before script execution.
