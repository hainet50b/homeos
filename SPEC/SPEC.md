...
- **Windows binary linkage**: builds for the `*-pc-windows-msvc` targets statically link the MSVC C runtime (`-C target-feature=+crt-static` via `.cargo/config.toml`), so `homeos.exe` runs on a fresh Windows machine without the Visual C++ Redistributable (`vcruntime140.dll`). Linux and macOS targets are unaffected.
