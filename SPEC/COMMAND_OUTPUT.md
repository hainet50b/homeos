...
| git binary not found (error) | stderr | `Error: git not found on PATH. homeos requires Git 2.28+.` (reason: `git-not-found`) |
...
| Newer release available (data directory exists, check not skipped) | stderr | `homeos: {latest} available — update at https://github.com/hainet50b/homeos` (single line, written after the guide; same format as `homeos cd`) |
