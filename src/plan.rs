...
impl Plan {
    ...
    /// Check enabled packages for unmodified skeleton scripts and populate warnings.
    pub fn check_unmodified_scripts(&mut self, packages_dir: &Path, config: &Config) {
        for name in &self.enabled {
            if let Some(pkg_config) = config.packages.get(name) {
                let script_name = resolve_script_name(pkg_config, self.action);
                let script_path = packages_dir.join(name).join(&script_name);
...
Skipping docker (disabled)";
    ...
    #[test]
    fn test_check_unmodified_scripts_detects_skeleton() {
        // Arrange
        ...
        let config = fixture_config(vec![("neovim", true)]);
        let mut plan = Plan::build(&config, &["neovim".to_string()], Action::Install, &[]).unwrap();

        // Act
        plan.check_unmodified_scripts(&packages_dir, &config);

        // Assert
        assert!(plan.warnings.contains_key("neovim"));
        assert_eq!(plan.warnings["neovim"].len(), 1);
        assert!(plan.warnings["neovim"][0].contains(&format!("install.{ext}")));
        assert!(plan.warnings["neovim"][0].contains("unmodified"));
    }

    #[test]
    fn test_check_unmodified_scripts_ignores_modified_scripts() {
        // Arrange
        let tmp = tempfile::TempDir::new().unwrap();
        let packages_dir = tmp.path().join("packages");
        let pkg_dir = packages_dir.join("neovim");
    ...
    #[test]
    fn test_check_unmodified_scripts_ignores_missing_scripts() {
        // Arrange
        let tmp = tempfile::TempDir::new().unwrap();
        let packages_dir = tmp.path().join("packages");
    ...
    #[test]
    fn test_check_unmodified_scripts_with_action_override() {
        // Arrange
        let tmp = tempfile::TempDir::new().unwrap();
        let packages_dir = tmp.path().join("packages");
        let pkg_dir = packages_dir.join("neovim");
    ...
    #[test]
    fn test_check_unmodified_scripts_skips_disabled_packages() {
        // Arrange
        let tmp = tempfile::TempDir::new().unwrap();
        let packages_dir = tmp.path().join("packages");
        let pkg_dir = packages_dir.join("neovim");
