...
- [ ] In install/update/apply plans, skip any package whose dependency chain includes a disabled package, with annotation `(dependency disabled: <name>)` where `<name>` is the most direct unavailable dependency. Currently if A depends on B and B is disabled, B is correctly skipped with `(disabled)` but A is still executed, which is likely to fail because A's install/update script relies on B being installed. After classifying disabled packages in `Plan::build`, perform a propagation pass: walk each enabled package's `depends_on` graph; if any dependency is disabled (directly or transitively skipped via this same rule), classify the package as skipped with `(dependency disabled: <most_direct_unavailable_dep>)`. Apply to `homeos package install`, `homeos package update`, and `homeos apply`. Uninstall is unaffected because it ignores disabled status. Update `Plan`, `Plan::display`, the Plan Display section of COMMAND_OUTPUT.md, and tests accordingly.
