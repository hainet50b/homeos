...
- [ ] Update `test-command-output.sh` for the flat data directory and `HOMEOS_DATA_DIR`-based isolation. Set `HOMEOS_DATA_DIR="$(mktemp -d)"` at the top of the script to isolate the test run from the developer's real homeos data, and remove all `--repo "$TEST_REPO"` arguments and the `$TEST_REPO` variable. Replace the `homeos repo add "$TEST_REPO"` setup and `homeos repo remove "$TEST_REPO"` teardown with plain `mkdir`/`rm -rf` against `$HOMEOS_DATA_DIR`. Remove the trailing `=== homeos repo list ===`, `=== homeos repo add (already exists) ===`, and `=== homeos repo remove (default) ===` example sections from the script.
