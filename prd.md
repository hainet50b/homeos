...
- [ ] Extract git clone invocation into a shared helper. Create a `git` module with a `clone(url, target)` function that wraps the `Command::new("git")` call and error handling (`git clone failed: {stderr}`). Migrate `homeos init`, `homeos repo add`, and `homeos plugin add` to use it.
