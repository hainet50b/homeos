...
- [ ] Move `check_unmodified_scripts` logic into `Plan::build` so that warnings are computed at construction time. Remove `&mut self` method and make `Plan` immutable after build.
