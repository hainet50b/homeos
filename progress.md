...
# Ralph Loop — 2026-05-19T07:51:06Z

## Task: Force `core.autocrlf=false` in git invocations

**Timestamp:**

2026-05-19T07:53:40Z

**Why this task:**

The single remaining unchecked task in the PRD (#246). It was added in the last commit (`cd3a6b2`) specifically to repair the Windows `Build` workflow that has been red since v0.3.2 — `homeos plugin refresh`'s `compute_diff` reports every cloned plugin as "modified" on Windows because Git for Windows ships with `core.autocrlf=true` and rewrites LF to CRLF on checkout. With #246 cleared, the PRD's `## Tasks` and `## Post Tasks` lists are both fully checked off, so this loop also closes out the v0.3.5 PRD.

**What was done:**

1. **`src/git.rs::clone`** — prepended `-c core.autocrlf=false` to the argv before the `clone` subcommand. The final invocation is `git -c core.autocrlf=false clone <url> <target>`. Git's `-c key=value` form attaches a command-line scoped config that overrides system + global + local, so any user with `core.autocrlf=true` in their global gitconfig (Git for Windows default) gets LF-preserving clones for homeos repositories and plugins regardless.

2. **`src/git.rs::init`** — same prefix: `git -c core.autocrlf=false init --initial-branch=main <target>`. This is defense-in-depth — `init` itself doesn't filter content, but pinning the override on every git invocation in this module means future maintenance won't accidentally introduce a non-pinned call site and the rule is uniform across the module surface.

3. **Regression test `test_clone_preserves_lf_when_global_config_requests_autocrlf`.** Sets up a local source repo containing `.gitattributes` (`* text`) and `script.sh` (LF-terminated `echo hi\n`), commits both, then points `GIT_CONFIG_GLOBAL` at a temp gitconfig file containing `[core] autocrlf = true` to simulate the Git for Windows-style global default. The test calls `clone()` and asserts the cloned `script.sh` is byte-identical to the source (`b"echo hi\n"` with LF preserved). If the `-c core.autocrlf=false` override were dropped from `git::clone`, this test would fail on any platform because git's clone-time checkout would consult the simulated global config and rewrite LF → CRLF.

4. **Env var isolation.** The test uses the existing `crate::env_test::EnvVarGuard` helper to capture and restore `GIT_CONFIG_GLOBAL`. This guard acquires the module-wide env-lock mutex so the test serializes against the other env-touching tests (`Context::new` tests for `HOMEOS_DATA_DIR`, completer tests, etc.) and never leaks `GIT_CONFIG_GLOBAL` to subsequent tests in the same `cargo test` process.

5. **No init-side test.** `git init` doesn't perform a checkout, so `core.autocrlf` has no observable effect at init time on the working tree. Writing a behavioral assertion against init would require either (a) following up with a commit-and-checkout dance just to exercise the filter, which tests too many moving parts at once, or (b) parsing the argv passed to `Command`, which requires changing the function's public surface to inject a fake. Neither felt worth the extra complexity for what is a defense-in-depth pin on init. The clone test exercises the same library-level pattern (`-c core.autocrlf=false` prefix on a `Command::new("git")` invocation) and provides high confidence that the maintainer's mental model is correct; init's prefix follows the same pattern by code review.

**What was changed:**

- src/git.rs — prepended `-c core.autocrlf=false` to both `clone` and `init` argvs; added the regression test `test_clone_preserves_lf_when_global_config_requests_autocrlf` in the clone-test cluster, between `test_clone_succeeds_for_valid_local_repo` and `test_clone_fails_for_invalid_url`; added `use crate::env_test::EnvVarGuard;` to the test module.
- prd.md — task #246 checked off.
- progress.md — this entry.

**Remarks:**

- **All 773 tests pass** (was 772, +1 new). `cargo fmt`, `cargo clippy --all-targets -- -D warnings`, and `cargo test` are all clean.
- **Why `-c core.autocrlf=false` and not a different config knob.** There are a few related knobs that influence line-ending behavior on checkout: `core.autocrlf`, `core.eol`, and `core.safecrlf`. The relevant one for the Git for Windows installer default is `autocrlf=true` — it activates the LF→CRLF rewrite on checkout (and the CRLF→LF rewrite on commit). Setting `autocrlf=false` disables both rewrites and is the only knob we need to pin. `core.eol` is only consulted when `autocrlf` is set to `input` or unset, so once `autocrlf` is forced off, `eol` is moot. `safecrlf` is about detecting irreversible line-ending changes during commit, not preventing them. So `autocrlf=false` is necessary and sufficient.
- **Why `-c key=value` rather than a per-invocation `GIT_CONFIG_*` env var.** Both approaches work — git supports `GIT_CONFIG_COUNT` / `GIT_CONFIG_KEY_N` / `GIT_CONFIG_VALUE_N` env vars for the same effect. The `-c` form is preferable here because (1) it scopes the override exactly to the single `Command` we're constructing without touching the process's environment, so it doesn't interact with concurrently-running tests via env-var serialization; (2) it makes the override visible at the call site in `args([...])` rather than buried in `.env(...)` calls, which a future maintainer is more likely to keep intact when adding new args; (3) it's syntactically lighter — one `-c key=value` token vs. three env vars.
- **Why the test uses `GIT_CONFIG_GLOBAL` rather than `GIT_CONFIG_COUNT`+keys.** Both simulate "global config says autocrlf=true." `GIT_CONFIG_GLOBAL` is one env var (cleanly handled by the existing `EnvVarGuard::capture` API), versus `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_0`/`GIT_CONFIG_VALUE_0` which would need three separate guards or an extension of the helper. The single-var approach also more faithfully reproduces the actual failure mode — Git for Windows users have a *global* gitconfig that sets autocrlf=true, not three orphan env vars.
- **Why the test creates `.gitattributes` with `* text` explicitly.** Without it, git's auto-classification of `script.sh` as text-or-binary depends on heuristics that can vary by git version. By committing `.gitattributes` with `* text` we pin classification so the test exercises exactly the LF-rewrite path that autocrlf=true triggers for text files. This makes the regression test robust against future git releases that might change the auto-text heuristic.
- **Why I did NOT also pass `-c core.eol=lf`.** Defense-in-depth concerns suggested pinning `core.eol=lf` as well, but per the autocrlf semantics above, `core.eol` is only consulted when `core.autocrlf` is `input` or unset — once we force `autocrlf=false`, `eol` has no effect. Adding it would be noise without behavioral change, and the PRD task description explicitly mentions only `core.autocrlf=false` as the fix.
- **Function ordering audit.** `src/git.rs` has two public functions, `clone` (line 5) before `init` (line 30). The README has no command named `git clone` / `git init` — the `git` module is internal-only infrastructure used by `homeos init`, `homeos repo add`, `homeos plugin add`, and `homeos plugin refresh`. So the README ordering rule doesn't apply to this module; function order is justified locally by "clone is used first in the codebase historically and is the more frequently called of the two." Within the test module, the new test slots into the clone-test cluster between `test_clone_succeeds_for_valid_local_repo` (the happy-path baseline) and `test_clone_fails_for_invalid_url` (the negative case). The progression is: happy path → autocrlf override regression → invalid URL error. Init tests are unchanged and remain after the clone tests.
- **3A pattern.** The new test has explicit Arrange / Act / Assert comments. Arrange creates the source repo (init, config, write files, commit), writes the simulated global gitconfig, and captures `GIT_CONFIG_GLOBAL`. Act is a single direct call to `clone(...)`. Assert reads the cloned file's bytes and compares to the LF-only baseline. The `run_git` closure inside Arrange is a local helper for repetitive git command invocations on the source repo — it does NOT invoke the unit under test. The unit under test (`clone`) is only called from Act.
- **No COMMAND_OUTPUT.md or README.md updates needed.** Per the PRD task description: "No user-facing output changes, so `COMMAND_OUTPUT.md` and `README.md` need no updates." The `-c core.autocrlf=false` override is invisible to the user — clones and inits produce the same on-disk state on Linux/macOS as before, and on Windows they now produce LF-preserving clones (which is the *intended* user-visible state per the existing scripts shipped with the project; the previous Windows behavior was a defect, not documented behavior).
...
# Ralph Loop — 2026-05-20T06:15:49Z

## Task: Rename-then-replace pattern in install.ps1

**Timestamp:**

2026-05-20T06:15:49Z

**Why this task:**

Last unchecked task in the PRD. Task #248 was completed in the previous loop iteration, leaving only #249. Single-file scope — touches only `install.ps1` — and Linux/macOS install.sh is explicitly out of scope per the PRD ("Linux / macOS file semantics already allow replacing a running binary because the existing inode keeps the running process alive while a new inode takes the path").

**What was done:**

Replaced the direct `Move-Item -Force` overwrite in `install.ps1` (line 55) with a rename-then-replace sequence:

1. **Capture the target path once.** Introduced `$TargetExe = Join-Path $InstallDir "homeos.exe"` so the destination path is computed in one place and reused for the rename guard, the move, the post-install cleanup scan, and the success message.
2. **Rename the existing binary if it exists.** When `Test-Path $TargetExe` returns true, generate an 8-hex suffix from `[guid]::NewGuid().ToString("N").Substring(0, 8)` and rename `homeos.exe` to `homeos.exe.old-<8-hex>` via `Rename-Item -ErrorAction Stop`. Windows allows renaming a running `.exe` even when it forbids overwriting one — this is the same pattern Chrome, VS Code, and most production Windows installers use.
3. **Surface a clear actionable error if the rename itself fails.** Wrapped the `Rename-Item` in `try/catch` and `throw` on failure with a message naming the path, explaining the likely cause (antivirus or a specific shell history handle), and telling the user exactly what to do (close running `homeos` processes and any terminals where homeos was recently invoked, then re-run). The original `$_` is appended for forensic context.
4. **Move the new binary into the now-vacant slot.** `Move-Item -Path (Join-Path $TempDir "homeos.exe") -Destination $TargetExe -Force` runs after the rename has confirmed the slot is free. `-Force` is retained but is now functionally redundant on the rename-then-replace path (slot is empty); it remains as a safety net for the fresh-install case where no prior binary existed.
5. **Sweep stale `homeos.exe.old-*` files after a successful install.** `Get-ChildItem -Path $InstallDir -Filter "homeos.exe.old-*"` enumerates leftovers from any prior installation, including the one we just created. Each is removed via `Remove-Item -Force -ErrorAction Stop` inside a per-file `try/catch` that silently swallows failures — exactly matching the PRD: "On a Remove-Item failure (the user is still running the previous version somewhere), silently ignore and let the next install clean up."
6. **PRD** — task #249 marked checked.

**What was changed:**

- install.ps1 — replaced single `Move-Item -Force` with the rename-then-replace sequence and post-install cleanup loop; introduced `$TargetExe` local. The success message now prints `$TargetExe` instead of the inlined `$InstallDir\homeos.exe` string for consistency.
- prd.md — task #249 checked off.
- progress.md — this entry.

**Remarks:**

- **All 783 Rust tests pass.** `cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, and `cargo test` are all clean. No Rust code was touched; the test run is a smoke test that confirms the workspace is consistent.
- **No unit tests added.** `install.ps1` has no automated test harness in the project (same precedent as tasks #219, #226, and #248). Adding Pester scaffolding for a single new rename branch would be infrastructure work disproportionate to the change. The behavior is straightforward and the failure mode is well-documented by Microsoft (NTFS lock semantics on `MoveFileEx` for running executables). Manual verification on a Windows host with `homeos.exe` open in another terminal is the appropriate confirmation, performed outside the Ralph loop by the maintainer or the next CI Windows build.
- **Why I generate the 8-hex suffix with `[guid]::NewGuid().ToString("N").Substring(0, 8)`.** PowerShell 5.1 has no `Get-Random -Format Hex` or equivalent one-liner that produces a stable lowercase hex string. `[guid]::NewGuid().ToString("N")` is GUID v4 (cryptographically-random per Microsoft docs), `"N"` produces no dashes, lowercase. Taking the first 8 chars yields 4 billion-ish unique suffixes which is far more than enough — collisions between concurrent installer runs are essentially impossible, and even if they collided, the second rename would fail and the user would get a clear retry message. Using `Get-Date -Format "yyyyMMddHHmmss"` was rejected because two re-runs within the same second (e.g., a user spamming the install command after a transient network failure) would collide.
- **Why `Remove-Item` failures are swallowed silently.** The PRD requires it ("silently ignore and let the next install clean up"). A `Write-Warning` or `Write-Host` on cleanup failure would create unjustified noise — every Windows machine with a long-running terminal session is expected to accumulate a small handful of stale `.old-*` files until those terminals close. The next install successfully cleans them up because the previous process has exited by then. The user never sees the leftover files unless they manually browse `$InstallDir`.
- **Why the success-message format change (`"Installed homeos to $TargetExe"` vs the previous `"Installed homeos to $InstallDir\homeos.exe"`).** Both expand to the same string at runtime (PowerShell `Join-Path` with `\` separator), so the user-visible output is byte-identical. Using `$TargetExe` keeps the source DRY — the path is constructed once in `$TargetExe`, reused everywhere. This is purely an internal-quality improvement; no behavior change.
- **Why `Move-Item -Force` is kept on the line that installs the new binary.** The PRD doesn't ask for its removal, and dropping `-Force` would break the fresh-install case if `$TargetExe` were to exist for any unrelated reason (e.g., a partial install leftover that bypassed our rename-then-replace guard, or a file at that path created by another tool). The `-Force` is now a defensive belt-and-suspenders rather than the load-bearing mechanism for handling lock conflicts. The rename above is the actual lock-conflict fix.
- **Why the cleanup loop uses `-Filter "homeos.exe.old-*"`.** `Get-ChildItem` `-Filter` is forwarded to the underlying filesystem API and is dramatically faster than `Where-Object { $_.Name -like "homeos.exe.old-*" }` because the filter is applied during enumeration rather than after. For typical install directories this is microseconds-vs-milliseconds, but it also avoids materializing every file in `$InstallDir` into a PowerShell pipeline (matters if the user has co-located other tools in `~/.homeos/bin`).
- **Why I use `Test-Path $TargetExe` rather than `[System.IO.File]::Exists($TargetExe)`.** `Test-Path` is idiomatic PowerShell and handles the case where `$TargetExe` resolves to a directory or a junction — both of which would be operator error but `Test-Path` returns true for them and would let the rename proceed normally (and fail cleanly if the rename itself can't operate on a junction). `[System.IO.File]::Exists` returns false for directories, which would skip our rename guard and then `Move-Item -Force` would try to overwrite the directory — an obscure failure mode worth defending against.
- **Why I do NOT add a `--cleanup` flag or similar to manually purge old files.** YAGNI. The cleanup happens automatically on every successful install. Users who run the installer regularly enough for stale `.old-*` files to matter will see them disappear naturally. Users who never re-run the installer have a single `.old-*` file at worst, taking up a few MB on disk. Adding a manual purge command is unjustified surface area.
- **Function/method/CLI ordering audit.** README maps user-visible homeos command order; `install.ps1` is a bootstrap entrypoint not documented as a command in README. The new logic is inline within the existing `try` block, in the order: directory creation → rename guard → move → cleanup → success message. This is the natural sequence of operations and matches how Chrome's installer is documented to work. No CLI surface changes.
- **README.md was NOT modified.** Task #249 is silent on README updates. The user-visible install command (`irm https://raw.githubusercontent.com/hainet50b/homeos/main/install.ps1 | iex`) is unchanged. The new rename behavior is invisible on first install (no prior `homeos.exe` exists) and invisible on subsequent re-runs from a closed-binary state. Only the locked-binary failure path's error message changes — and that's an error string, not documented user-visible output.
- **COMMAND_OUTPUT.md was NOT modified.** That file specifies homeos *command* output, not installer script output. The installer's error message follows installer conventions (echo / Write-Host / throw), not the homeos command conventions tracked there.
